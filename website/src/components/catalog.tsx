...
export function Catalog({ kind }: { kind: "solutions" | "devices" }) {
  ...
  return <div className="catalog wrap">
    ...
    {matching.length > 0 ? <div className={isSolutions ? "solution-grid" : "device-grid"}>{matching.map(item => {
      ...
      return <article className="device-card" key={item.title}><div className="device-icon"><Icon size={24} weight="regular" aria-hidden="true" /><span className="mono">{item.category}</span></div><h2>{item.title}</h2><p>{item.description}</p><Link href={`/solutions/${item.solution}`} className="card-link">Explore application<ArrowUpRight size={18} aria-hidden="true" /></Link></article>;
