...
export function Catalog({ kind }: {
 ...
 const t = useCopy();
    ...
    {matching.length > 0 ? <div className={isSolutions ? "solution-grid" : "device-grid"}>{matching.map(item => {
                if ("slug" in item)
                    return <Link className="solution-card" href={`/solutions/${item.slug}`} key={item.slug}><div className="solution-card-header"><span className="solution-card-number" aria-hidden="true">{item.number}</span><span className="mono">{t(item.category)}</span></div><div className="solution-card-body"><h2>{t(item.title)}</h2><p>{t(item.description)}</p><span className="card-link">{t("Explore solution")}<ArrowUpRight size={20} aria-hidden="true"/></span></div></Link>;
