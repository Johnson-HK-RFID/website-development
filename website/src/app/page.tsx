...
export default function Home() {
  return <>
    <section className="home-hero wrap">
      ...
      <div className="hero-visual"><ConceptImage priority /><div className="hero-image-note"><span className="mono">THE BUILT WORLD, CONNECTED</span></div></div>
    </section>

    <section className="value-strip"><div className="wrap value-strip-inner"><span className="value-strip-label">From the field.<br /><strong>To the bigger picture.</strong></span><div><span>Connect devices</span></div><ArrowRight className="strip-arrow" size={18} aria-hidden="true" /><div><span>Make data useful</span></div><ArrowRight className="strip-arrow" size={18} aria-hidden="true" /><div><span>Capture evidence</span></div></div></section>

    <section className="section wrap"><div className="section-heading"><h2>Real environments.<br /><span className="muted-heading">Connected solutions.</span></h2><p>Start with the challenge on your site. Build the right combination of devices, engineering and intelligence.</p></div><div className="home-solutions">{solutions.filter(s => s.slug !== "outrigger-monitoring").map((solution, i) => <Link key={solution.slug} href={`/solutions/${solution.slug}`} className={`home-solution ${i === 0 ? "home-solution-featured" : ""}`}><div className="solution-topline"><span className="mono">{solution.category}</span><ArrowUpRight size={20} aria-hidden="true" /></div><div><h3>{solution.title}</h3><p>{solution.description}</p></div></Link>)}</div><div className="section-bottom"><TextLink href="/solutions">View all solutions</TextLink><Link className="subtle-link" href="/traci#plugins">Explore TRACI BCDS and the plugin family <ArrowRight size={16} aria-hidden="true" /></Link></div></section>
