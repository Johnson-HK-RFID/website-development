...
export default function Home({params}: {params: Promise<{locale: string}>}) {
 ...
 const t = useCopy();
    ...
    <section className="services-section"><div className="wrap services-grid"><div className="section-heading"><h2>{t("Three ways")}<br />{t("to move forward.")}</h2><p>{t("Use the engineering you need.")}<br />{t("Add the hardware that fits.")}<br />{t("Connect it with managed services.")}</p><TextLink href="/services">{t("Explore our services")}</TextLink><FieldPhoto scene="site" className="services-inline-photo"/></div><ServiceList /></div></section>
