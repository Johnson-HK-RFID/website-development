...
export default async function SolutionPage({ params }: {
 ...
 const t = await getCopy();
    ...
    const scenes: Record<string, PageHeroScene> = { "gas-monitoring": "site", hookcam: "tower", "outrigger-monitoring": "infrastructure", "worker-tracking": "workers", "site-vision": "buildings", "rfid-asset-tracking": "industrial" };
    return <><section className="page-intro solution-page-intro wrap"><nav className="breadcrumb" aria-label={t("Breadcrumb")}><Link href="/solutions">{t("Solutions")}</Link><ArrowRight size={13} aria-hidden="true"/><span>{t(solution.title)}</span></nav><div className="detail-intro"><div><div className="eyebrow">{t(solution.category.toUpperCase())}</div><h1>{t(solution.title)}</h1><p>{t(solution.description)}</p><div className="intro-actions"><ActionLink href={`/contact?service=${encodeURIComponent(solution.title)}`}>{t("Let\u2019s talk")}</ActionLink></div></div><div className="detail-visual"><span aria-hidden="true">{solution.number}</span><div>{solution.focus.map(focus => <span className="tag" key={focus}>{t(focus)}</span>)}</div></div></div><PageHeroImage scene={scenes[solution.slug]}/></section>
