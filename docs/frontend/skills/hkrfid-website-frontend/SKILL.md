...
- The user has requested open-license engineering photography. Follow [PHOTOGRAPHY.md](../../../PHOTOGRAPHY.md): use individually verified sources, local responsive assets and accurate scene descriptions; preserve source and licence records internally. Do not place stock-library attribution captions in the public interface.
...
- The user's later Imprezz reference explicitly supersedes click-to-play video. Use motion intensity 6: a muted, looping, inline construction timelapse without visible homepage playback chrome, once-per-visit staggered entrances, slow editorial image movement, reading progress and purposeful hover/focus feedback. Provide reduced-motion and data-saving poster fallbacks; retain visible server-rendered content and avoid simulated-live interfaces.
